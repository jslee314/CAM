...
class Layers:
    ...
    def batch_norm(self, inputs, act=tf.nn.relu6, is_training=True, name='batch_norm_layer'):
        '''
            Batch Normalization
             - scale=True, scale factor(gamma) �� ���
             - center=True, shift factor(beta) �� ���
        '''
        with tf.variable_scope(name_or_scope=name):
            return tf.contrib.layers.batch_norm(inputs=inputs, decay=0.9, center=True, scale=True, fused=True,
                                                activation_fn=act, is_training=is_training, scope='batch_norm')
    ...
    def inverted_bottleneck(self, inputs, filters, strides, repeat, factor, is_training, name=None):
        def _mobilenet_block(inputs, input_filters, output_filters, strides, name):
            with tf.variable_scope(name_or_scope=name):
                ...
                layer = self.batch_norm(inputs=layer, is_training=is_training, name='bottleneck_batch')
                ...
                layer = self.batch_norm(inputs=layer, is_training=is_training, name='depthwise_batch')
                ...
                layer = self.batch_norm(inputs=layer, act=None, is_training=is_training, name='linear_batch')
        ...
        with tf.variable_scope(name_or_scope=name):
            for idx in range(repeat):
                ...
                '''inverted_bottleneck ���� ù ��° layer �� strides=2 �� ��� shortcut connection ����'''
...
class Model(Layers):
    ...
    def build(self):
        with tf.variable_scope(name_or_scope=self.name):
            ...
            with tf.variable_scope(name_or_scope='mobilenet_v2_module'):
                ...
                layer = self.batch_norm(inputs=layer, is_training=self.training, name='conv2d_0_batch')
                # layer = self.group_norm(inputs=layer, name='conv2d_0_group')
                # layer = tf.nn.relu6(layer, name='conv2d_0_group')
                layer = self.inverted_bottleneck(inputs=layer, filters=16, strides=1, repeat=1, factor=1, is_training=self.training, name='bottleneck_1')
                layer = self.inverted_bottleneck(inputs=layer, filters=24, strides=2, repeat=2, factor=6, is_training=self.training, name='bottleneck_2')
                layer = self.inverted_bottleneck(inputs=layer, filters=32, strides=2, repeat=3, factor=6, is_training=self.training, name='bottleneck_3')
                layer = self.inverted_bottleneck(inputs=layer, filters=64, strides=2, repeat=4, factor=6, is_training=self.training, name='bottleneck_4')
                layer = self.inverted_bottleneck(inputs=layer, filters=96, strides=1, repeat=3, factor=6,is_training=self.training, name='bottleneck_5')
                layer = self.inverted_bottleneck(inputs=layer, filters=160, strides=2, repeat=3, factor=6, is_training=self.training, name='bottleneck_6')
                layer = self.inverted_bottleneck(inputs=layer, filters=320, strides=1, repeat=1, factor=6, is_training=self.training, name='bottleneck_7')
                layer = self.conv2d(inputs=layer, filters=1280, name='conv2d_8')
                layer = self.batch_norm(inputs=layer, is_training=self.training, name='conv2d_8_batch')
                self.cam_layer = layer
                # layer = self.group_norm(inputs=layer, name='conv2d_8_group')
                # layer = tf.nn.relu6(layer, name='conv2d_8_group')
                layer = tf.layers.dropout(inputs=layer, rate=flags.FLAGS.dropout_rate, training=self.training, name='conv2d_8_dropout')
                layer = tf.reduce_mean(input_tensor=layer, axis=[1, 2], keep_dims=True, name='global_pool')
                layer = tf.layers.dropout(inputs=layer, rate=flags.FLAGS.dropout_rate, training=self.training, name='conv2d_9_dropout')
                layer = self.conv2d(inputs=layer, filters=2, name='conv2d_9')
                self.logits = tf.squeeze(input=layer, name='squeeze')  # (batch_size, class)
    ...
    def validate(self, x, y, training):
        return self.sess.run([self.accuracy, self.loss], feed_dict={self.x: x, self.y: y,
                                                                    self.training: training})

    def test(self, x, training):
        return self.sess.run(self.logit, feed_dict={self.x: x, self.training: training})